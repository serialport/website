...
title: 📦 stream
---
import StreamAPIOpenOptions from './_shared/stream-api-open-options.mdx';
...
Opens the connection of the given serial port. Emits an [`open`](api-stream#open) event when the port is open, and if provided, calls the callback function after emitting. If the serial port's path does not exist, the callback is invoked with an error message. If no callback handler is provided, the `SerialPort` object will emit an [`error`](api-stream#error) event.
